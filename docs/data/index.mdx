...
Numbani-react take the assumption that most actions an user take on your web application, could be reduced to **CRUD** (**C**reate, **R**ead, **U**pdate and **D**elete) operations on **entities** (also called models).
...
Thus, to make it easier to build web app, we provide you the **EntityManager** abstraction.
...
The persistence provider is the underlying backend that allow you to persist your data (entities).
...
import EntityManagerBuilder from 'numbani-react/model/EntityManagerBuilder';
import FirestorePersistence from 'numbani-react/model/persistence/provider/Firestore';
...
Once you have an entity manager, next step is to add some entities... and well manage them 😄.
...
import EntitySchemaBuilder from 'numbani-react/model/schema/EntitySchemaBuilder';
import types from 'numbani-react/model/schema/types';
